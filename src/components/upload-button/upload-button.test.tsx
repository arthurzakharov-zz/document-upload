...
describe("UploadButton", () => {
  ...
  test("Snapshot is in not loaded state", () => {
    ...
    const { container } = render(<UploadButton loaded={false} onClick={mockFn} />);
    expect(container.firstChild).toMatchSnapshot();
  });
  test("Snapshot is in loaded state", () => {
    const mockFn = jest.fn();
    const { container } = render(<UploadButton loaded onClick={mockFn} />);
    expect(container.firstChild).toMatchSnapshot();
  });
  test("In not loaded state upload icon and text are displayed", () => {
    const mockFn = jest.fn();
    render(<UploadButton loaded={false} onClick={mockFn} />);
  ...
  test("In loaded state check icon is diplayed", () => {
    const mockFn = jest.fn();
    render(<UploadButton loaded onClick={mockFn} />);
